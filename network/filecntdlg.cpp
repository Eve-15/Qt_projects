...
!= 0))
    ...
    ui->rfileSizeLineEdit->setText(tr("%1").arg(myTotalBytes / (1024 * 1024)) + " MB");										//  д ļ   С
    ui->recvSizeLineEdit->setText(tr("%1").arg(myRcvedBytes / (1024 * 1024)) + " MB");										//  д ѽ
    ui->rateLabel->setText(tr("%1").arg(transpeed * 1000 / (1024 * 1024), 0, 'f', 2) + " MB/ 秒 ");								//   㲢  ʾ
    if(myRcvedBytes == myTotalBytes)
    {
        myLocPathFile->close();
        myCntSocket->close();
